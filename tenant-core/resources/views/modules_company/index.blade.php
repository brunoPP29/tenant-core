<x-layouts::app :title="__('Módulos')">
    ...
    @php
        $companyModule = $companyModules->firstWhere('module_id', $module->id);
    @endphp
    ...
    <div class="relative aspect-video overflow-hidden rounded-xl border border-neutral-200 bg-white p-6 shadow-sm transition-all dark:border-neutral-700 dark:bg-neutral-800 flex flex-col justify-between">
        ...
        <div class="z-10 mt-4 flex gap-2">
            ...
            @else
                ...
                <form method="POST" action="{{ route('modulesCompany.activate', $module->id) }}">
                    @csrf
                    @method('PATCH')
                    <button type="submit"
                        class="flex-1 rounded-lg border border-green-200 px-3 py-2 text-xs font-semibold text-green-600 hover:bg-green-50 dark:border-green-800 dark:text-green-400 dark:hover:bg-green-900 transition-all active:scale-95">
                        Ativar
                    </button>
                </form>
